...
def main():
    ...
    cwd = Path().absolute()
    data_file = cwd / Path('tutorials/data/test_data/example_diff_express_data.txt')
    ...
    # print(out.head(3))
    # id_to = 'Symbol'
    # id_from = 'Human NCBI Gene ID'
    # gene_id_list = [100000252,100000750, 100001198, 100001260, 100002225, 100002263,100002756]

    # gene_id_list = pd.read_csv(data_file, sep = '\t')
    # out = get_ortho_ids(gene_id_list, id_from, id_to)
    # print(out)
    # list_of_human_ids = [55585, 23191, 4192, 5686, 
    #                  197021, 390, 344805, 2623]
    # zfish_desired_gene_id_type = 'Symbol'
    # zebrafish_ids = convert_to_zebrafish(list_of_human_ids, zfish_desired_gene_id_type)
    # print(zebrafish_ids)
    ...
    # human_ids = convert_to_human(list_of_zfish_ids, zfish_gene_type, False)
    # print(human_ids)
    data = pd.read_csv(data_file, sep='\t')
    ...
    # gene_list = data[id_from]
    # gene_list_with_orthos = get_ortho_ids(gene_list, id_from, id_to, keep_mapping=True)
    new_data = add_mapped_ortholog_column(data, id_from, id_to)
