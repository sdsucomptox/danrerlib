...
def test_get_genes_from_go_id():
    ...
    generated_genes = generated_genes.sort_values(by='Symbol', ascending=True).reset_index(drop=True)
    ...
    generated_genes = generated_genes.sort_values(by='Human NCBI Gene ID', ascending=True).reset_index(drop=True)
